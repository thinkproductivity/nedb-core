...
function Datastore (options) {
  ...
  if (typeof options === 'string') {
    filename = options;
    this.inMemoryOnly = true;   // Default
  } else {
    options = options || {};
  ...
  if (!filename || typeof filename !== 'string' || filename.length === 0) {
    ...
    if (!options.inMemoryOnly && !options.storage) {
      throw new Error("You must provide a 'storage' option when inMemoryOnly is not set to true.");
    }
