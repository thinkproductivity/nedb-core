 ...
 * For a React Native database it's the file system on the device
 *
 * This version is the React Native version that uses react-native-fs
 * under the hood.
 */

// TODO (EK): TO GET THIS WORKING
// ===============================
// 
// 1. Convert all promises to callbacks.
// 2. polyfill mkdirp possibly just using fs.mkdir and making it return a callback. DONE
// 3. polyfill path module. DONE
...
storage.rename = fs.moveFile;
storage.writeFile = fs.writeFile;
storage.unlink = fs.unlink;
storage.appendFile = fs.appendFile;
storage.readFile = fs.readFile;
storage.mkdirp = function(filepath, callback) {
  fs.mkdir(filepath)
    .then(() => callback())
    .catch(callback);
};
